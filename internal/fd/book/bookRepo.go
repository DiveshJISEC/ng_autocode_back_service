...
func (g *dbSt) GetFDOrderBook(c context.Context, dtoRequest *FDOrderBookRequest) (dtoResponse []*FDOrderBookResponse, e error) {
	logger.Log(c).Info("GetfdOrderBook called")

	// Create a slice to hold the results
	var orderBook []*FDOrderBookResponse

	// Query the database for FDorder list
	var fdOrderBook []FDOrderBook
	...
	for _, order := range fdOrderBook {
		orderBook = append(orderBook, &FDOrderBookResponse{
